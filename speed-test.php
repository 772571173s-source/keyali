...
<style>
    .speed-test-box { max-width: 900px; margin: 30px auto; text-align: center; font-family: 'Tajawal', sans-serif; padding: 0 15px; box-sizing: border-box; }
    ...
    .setup-group { display: flex; flex-direction: column; text-align: right; gap: 8px; width: 100%; max-width: 220px; }
    .setup-group label { font-size: 14px; color: #38bdf8; font-weight: bold; }
    .setup-select { padding: 10px 15px; background-color: #0f172a; border: 1px solid #334155; border-radius: 8px; color: #fff; font-size: 15px; outline: none; cursor: pointer; width: 100%; box-sizing: border-box; }
    .setup-select:focus { border-color: #38bdf8; }

    /* أزرار التحكم بالتحدي */
    .controls-wrapper { display: flex; gap: 15px; align-items: center; justify-content: center; flex-wrap: wrap; width: 100%; margin-top: 10px; }
    .start-trigger-btn { background-color: #4ade80; color: #020617; font-size: 16px; font-weight: bold; border: none; padding: 12px 30px; border-radius: 10px; cursor: pointer; transition: all 0.3s; min-width: 160px; }
    .start-trigger-btn:hover { background-color: #22c55e; transform: translateY(-2px); box-shadow: 0 0 15px rgba(74, 222, 128, 0.4); }
    
    /* زر الإيقاف الإضافي */
    .stop-trigger-btn { background-color: #ef4444; color: #fff; font-size: 16px; font-weight: bold; border: none; padding: 12px 25px; border-radius: 10px; cursor: pointer; transition: all 0.3s; display: none; min-width: 130px; }
    .stop-trigger-btn:hover { background-color: #dc2626; transform: translateY(-2px); box-shadow: 0 0 15px rgba(239, 68, 68, 0.4); }

    /* لوحة الإحصائيات الحية أثناء اللعب */
    .stats-bar { display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin-bottom: 25px; width: 100%; box-sizing: border-box; }
    .stat-card { background: #1e293b; border: 1px solid #334155; padding: 12px 10px; border-radius: 12px; box-shadow: 0 5px 15px rgba(0,0,0,0.2); text-align: center; }
    .stat-val { font-size: clamp(20px, 5vw, 28px); font-weight: bold; color: #38bdf8; }
    .stat-lbl { font-size: clamp(11px, 3vw, 13px); color: #94a3b8; margin-top: 5px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    ...
    .speed-word { color: #475569; padding: 2px 6px; border-radius: 6px; transition: all 0.2s ease; font-family: 'Fira Code', monospace; display: inline-block; word-break: break-all; }
    .speed-word.current { color: #38bdf8; background-color: rgba(56, 189, 248, 0.15); font-weight: bold; border-bottom: 2px solid #38bdf8; transform: scale(1.05); }
    .speed-word.correct { color: #4ade80; font-weight: 600; }
    .speed-word.wrong { color: #ef4444; text-decoration: line-through; }

    /* حقل الكتابة الاحترافي الكبير */
    .speed-input-container { width: 100%; display: flex; justify-content: center; margin-top: 25px; }
    .speed-input {
        width: 100%; max-width: 550px; padding: 15px 20px; font-size: clamp(16px, 4.5vw, 22px); font-family: 'Fira Code', monospace;
        background-color: #0f172a; border: 2px solid #334155; border-radius: 12px; color: #fff;
        text-align: center; outline: none; transition: all 0.3s ease; box-sizing: border-box;
    }
    .speed-input:focus { border-color: #38bdf8; box-shadow: 0 0 20px rgba(56, 189, 248, 0.3); }
    .speed-input:disabled { background-color: #1e293b; color: #94a3b8; cursor: not-allowed; border-color: #334155; }

    /* 📱 استجابة شاشات الهواتف والتابلت اللمسية */
    @media (max-width: 768px) {
        .speed-test-box h2 { font-size: clamp(18px, 5.5vw, 24px); }
        ...
        .setup-panel { padding: 15px; gap: 15px; flex-direction: column; }
        .setup-group { max-width: 100%; }
        .controls-wrapper { width: 100%; flex-direction: column; gap: 10px; }
        .start-trigger-btn, .stop-trigger-btn { width: 100%; }
        .stats-bar { gap: 8px; }
    }
</style>
...
<div class="speed-test-box">
    ...
    <div class="stats-bar">
        <div class="stat-card"><div class="stat-val" id="timer-val">60</div><div class="stat-lbl">المتبقي (ثانية)</div></div>
        <div class="stat-card"><div class="stat-val" id="wpm-val">0</div><div class="stat-lbl">كلمة / دقيقة</div></div>
        <div class="stat-card"><div class="stat-val" id="accuracy-val">100%</div><div class="stat-lbl">نسبة الدقة</div></div>
    </div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function startSpeedTest() {
        ...
        normalKeySound.play().then(() => { normalKeySound.pause(); normalKeySound.currentTime = 0; });
        spaceKeySound.play().then(() => { spaceKeySound.pause(); spaceKeySound.currentTime = 0; });
