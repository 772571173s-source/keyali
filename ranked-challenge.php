...
<style>
    ...
    .lang-selector {
        ...
        text-align: center;
        appearance: none;
        /* إخفاء سهم المتصفح الافتراضي المزعج */
...
<script>
    ...
    function startRankedArena() {
        ...
        if (selectedMode === "english") {
            if (engPool.length === 0) {
                alert("قائمة الكلمات الإنجليزية فارغة في قاعدة بياناتك!");
                return;
            }
            currentPool = [...engPool];
        } else {
            if (codePool.length === 0) {
                alert("قائمة الأكواد البرمجية فارغة في قاعدة بياناتك!");
                return;
            }
