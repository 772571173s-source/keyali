...
<head>
    ...
    <style>
        ...
        .nav-links a { text-decoration: none; font-size: 14px; transition: 0.2s; }
        .nav-links a:hover { opacity: 0.8; }

        .dashboard-container { max-width: 1200px; margin: 40px auto; padding: 0 20px; box-sizing: border-box; }
        
        /* صندوق الترحيب المتجاوب */
        .welcome-box { margin-bottom: 25px; border-bottom: 1px solid #334155; padding-bottom: 20px; text-align: right; }
        .welcome-box h2 { font-size: clamp(20px, 4vw, 28px); margin: 0 0 10px 0; font-weight: 700; }
        .welcome-box p { font-size: clamp(14px, 3vw, 16px); margin: 0; }
        
        .role-badge { display: inline-block; background: rgba(56, 189, 248, 0.1); color: #38bdf8; padding: 4px 10px; border-radius: 6px; font-size: 13px; font-weight: bold; margin-top: 5px; }

        /* ✨ الإضافة 1: قسم صناديق الإحصائيات الشاملة السريعة */
        .mini-stats-container { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 12px; margin-bottom: 30px; }
        .stat-mini-box { background: #0f172a; border: 1px solid #1e293b; padding: 15px; border-radius: 12px; text-align: center; }
        .stat-mini-box .stat-num { font-size: 20px; font-weight: 900; color: #38bdf8; block-size: auto; }
        .stat-mini-box .stat-label { font-size: 12px; color: #64748b; margin-top: 5px; font-weight: bold; }
        .stat-mini-box.total-box { border-color: rgba(74, 222, 128, 0.3); background: rgba(74, 222, 128, 0.02); }
        .stat-mini-box.total-box .stat-num { color: #4ade80; }

        /* ✨ الإضافة 2: شريط البحث والفلترة الذكي */
        .search-wrapper { position: relative; margin-bottom: 30px; display: flex; gap: 10px; width: 100%; box-sizing: border-box; }
        .search-input { flex-grow: 1; padding: 14px 20px; background: #0f172a; border: 1px solid #334155; border-radius: 12px; color: #fff; font-size: 15px; font-family: inherit; outline: none; transition: 0.2s; box-sizing: border-box; }
        .search-input:focus { border-color: #38bdf8; box-shadow: 0 0 15px rgba(56, 189, 248, 0.15); }
        .clear-search-btn { background: #1e293b; border: 1px solid #334155; color: #94a3b8; padding: 0 15px; border-radius: 12px; cursor: pointer; font-size: 14px; font-weight: bold; display: flex; align-items: center; justify-content: center; transition: 0.2s; }
        .clear-search-btn:hover { background: #334155; color: #fff; }

        /* شبكة التحكم (Grid) الموزعة بأناقة تامة لكل الشاشات */
        .cards-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(290px, 1fr)); gap: 20px; box-sizing: border-box; }
        .control-card { background: #1e293b; border: 1px solid #334155; border-radius: 16px; padding: 25px; transition: all 0.3s ease; text-decoration: none; color: #fff; display: flex; flex-direction: column; justify-content: space-between; box-sizing: border-box; }
        .control-card:hover { border-color: #38bdf8; transform: translateY(-4px); box-shadow: 0 10px 20px rgba(56, 189, 248, 0.08); }
        
        .card-icon { font-size: 30px; margin-bottom: 15px; display: flex; justify-content: space-between; align-items: center; }
        .card-title { font-size: 19px; font-weight: bold; color: #38bdf8; margin: 0 0 12px 0; }
        .card-desc { color: #94a3b8; font-size: 14px; margin: 0 0 25px 0; line-height: 1.6; flex-grow: 1; }
        
        /* تحسين مظهر وحجم أزرار التحكم لتناسب نقرات الأصابع الكبيرة من شاشة الهاتف */
        .card-btn { background: #0f172a; border: 1px solid #475569; padding: 12px; border-radius: 10px; text-align: center; font-size: 15px; font-weight: bold; color: #fff; transition: 0.2s; width: 100%; box-sizing: border-box; }
        
        .control-card:hover .card-btn { background: #38bdf8; color: #020617; border-color: #38bdf8; box-shadow: 0 0 10px rgba(56, 189, 248, 0.2); }
        
        .card-challenges:hover { border-color: #f59e0b; box-shadow: 0 10px 20px rgba(245, 158, 11, 0.1); }
        .card-challenges:hover .card-btn { background: #f59e0b; color: #020617; border-color: #f59e0b; box-shadow: 0 0 10px rgba(245, 158, 11, 0.2); }

        .live-counter { font-size: 13px; background: rgba(255, 255, 255, 0.05); color: #94a3b8; padding: 4px 12px; border-radius: 20px; border: 1px solid rgba(255, 255, 255, 0.1); font-weight: bold; }
        
        /* رسالة عدم وجود نتائج عند الفلترة */
        #no-results-msg { display: none; text-align: center; color: #64748b; padding: 40px 20px; font-weight: bold; font-size: 16px; background: #0f172a; border-radius: 16px; border: 1px dashed #334155; }

        /* 📱 شاشات الجوال والأجهزة الصغيرة جداً */
        @media (max-width: 768px) {
            .navbar { padding: 12px 15px; }
            .nav-container { justify-content: center; text-align: center; flex-direction: column; gap: 8px; }
            .dashboard-container { margin: 25px auto; padding: 0 15px; }
            .welcome-box { text-align: center; padding-bottom: 20px; margin-bottom: 20px; }
            .mini-stats-container { grid-template-columns: repeat(2, 1fr); gap: 10px; margin-bottom: 25px; }
            .search-wrapper { margin-bottom: 25px; }
            .cards-grid { grid-template-columns: 1fr; gap: 15px; } 
            .control-card { padding: 20px 15px; border-radius: 14px; }
            .card-btn { padding: 14px; font-size: 16px; } 
        }
    </style>
</head>
<body>
