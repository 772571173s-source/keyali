...
<style>
    ...
    body, .word-display, .word-card, #word-text {
    ...
    .ghost-word-container { 
        ...
        padding: 0 15px;
        flex-direction: row; /* افتراضي للكمبيوتر */
    }
    
    .game-box { flex: 1; max-width: 650px; text-align: center; width: 100%; }
    ...
    .ghost-leaderboard h3 { color: #c084fc; font-size: 19px; margin-bottom: 15px; text-align: center; }
    .ghost-rank-list { list-style: none; padding: 0; margin: 0; text-align: right; direction: rtl;}
    .ghost-rank-item { display: flex; justify-content: space-between; align-items: center; padding: 12px 10px; border-bottom: 1px solid rgba(124, 58, 237, 0.2); color: #f3e8ff; font-family: 'Fira Code', monospace; }
    .ghost-rank-item .badge-rank { background: #6d28d9; padding: 2px 7px; border-radius: 6px; font-size: 11px; }
    .no-ghosts-yet { text-align: center; color: #6b7280; font-size: 14px; padding: 20px 0; }

    .word-card { background: linear-gradient(135deg, #090514, #130b24); border: 2px solid #5b21b6; border-radius: 20px; padding: 50px 20px; margin-bottom: 25px; box-shadow: 0 15px 35px rgba(124, 58, 237, 0.15); position: relative; }
    
    /* خط الكلمة متجاوب مع حجم الشاشة */
    .word-display { font-family: 'Fira Code', monospace; font-size: clamp(28px, 5vw, 42px); font-weight: bold; color: #e9d5ff; letter-spacing: 1px; margin-bottom: 20px; direction: ltr; transition: opacity 0.4s ease, filter 0.4s ease; word-wrap: break-word; }
    .word-display.ghost-vanished { opacity: 0 !important; filter: blur(12px); pointer-events: none; }
    
    .ghost-timer { font-size: 15px; color: #f43f5e; font-weight: bold; margin-top: 5px; background: rgba(244, 63, 94, 0.1); display: inline-block; padding: 5px 15px; border-radius: 10px; }
    .ghost-timer.typing-time { color: #fbbf24; background: rgba(251, 191, 36, 0.1); }

    /* تعديل عرض عدادات الوقت لتناسب الموبايل بشكل لطيف دون تداخل */
    .timers-wrapper { display: flex; justify-content: center; gap: 10px; flex-wrap: wrap; margin-bottom: 20px; }
    .round-total-timer { font-size: 16px; color: #38bdf8; font-weight: bold; background: rgba(56, 189, 248, 0.15); border: 1px solid rgba(56, 189, 248, 0.4); padding: 8px 20px; border-radius: 50px; display: inline-block; }
    .ghost-streak-badge { font-size: 16px; color: #c084fc; background: rgba(139, 92, 246, 0.15); border: 1px solid rgba(139, 92, 246, 0.4); padding: 8px 20px; border-radius: 50px; display: inline-block; font-weight: bold; }

    .word-translation { display: inline-block; background: rgba(139, 92, 246, 0.1); color: #ddd6fe; padding: 8px 20px; border-radius: 30px; font-size: 16px; margin-top: 15px; border: 1px solid rgba(139, 92, 246, 0.2); }
    .controls-container { position: absolute; top: 15px; right: 15px; }
    .speaker-btn { background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); color: #22c55e; font-size: 18px; border-radius: 50px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; width: 40px; height: 40px; }

    .word-input { width: 100%; max-width: 400px; padding: 12px 15px; font-size: clamp(20px, 4vw, 26px); font-family: 'Fira Code', monospace; background-color: #0c0717; border: 2px solid #4c1d95; border-radius: 15px; color: #fff; text-align: center; outline: none; direction: ltr; box-sizing: border-box; }
    .word-input:focus { border-color: #a78bfa; box-shadow: 0 0 20px rgba(167, 139, 250, 0.3); }
    .word-input:disabled { background-color: #1a1528; color: #6b7280; cursor: not-allowed; }
    .word-input.input-error { border-color: #ef4444 !important; box-shadow: 0 0 15px rgba(239, 68, 68, 0.3); }
    
    .feedback-zone { height: 40px; font-size: clamp(16px, 3.5vw, 22px); font-weight: bold; margin-bottom: 20px; padding: 0 10px; }
    .text-success { color: #4ade80; }
    .text-error { color: #f87171; }

    .btn-start-adventure { background: linear-gradient(90deg, #7c3aed, #2563eb); color: #fff; padding: 12px 35px; font-size: clamp(16px, 4vw, 22px); font-weight: bold; border: none; border-radius: 50px; cursor: pointer; box-shadow: 0 0 25px rgba(124, 58, 237, 0.6); margin-bottom: 20px; transition: transform 0.2s; width: 90%; max-width: 350px; }
    .btn-start-adventure:hover { transform: scale(1.03); }
    .btn-skip { background-color: #312e81; color: #e0e7ff; padding: 12px 30px; font-size: 15px; border: none; border-radius: 10px; cursor: pointer; margin-top: 15px;}

    .rules-card { background: rgba(15, 10, 31, 0.7); border: 1px solid rgba(124, 58, 237, 0.3); border-radius: 15px; padding: 20px; text-align: right; max-width: 500px; margin: 0 auto 25px auto; direction: rtl; box-shadow: inset 0 0 20px rgba(124, 58, 237, 0.1); }
    .rules-card h4 { color: #c084fc; font-size: 17px; margin-top: 0; margin-bottom: 12px; display: flex; align-items: center; gap: 8px; }
    .rules-card ul { margin: 0; padding-right: 15px; color: #cbd5e1; font-size: 13px; line-height: 1.7; }
    .rules-card li { margin-bottom: 8px; }
    .highlight-gold { color: #fbbf24; font-weight: bold; }
    .highlight-purple { color: #a78bfa; font-weight: bold; }

    .game-over-box { background: linear-gradient(135deg, #1c0a21, #0a030d); border: 3px double #f43f5e; border-radius: 20px; padding: 30px 20px; box-shadow: 0 0 35px rgba(244, 63, 94, 0.3); max-width: 500px; margin: 30px auto; text-align: center; position: relative; width: 100%; box-sizing: border-box; }
    .server-status-msg { font-size: 13px; color: #a78bfa; margin-top: 10px; font-style: italic; }

    /* 🛡️ شاشات الموبايل (Media Queries) لجعل التصميم عمودياً ومنساباً */
    @media (max-width: 768px) {
        .ghost-word-container {
            flex-direction: column; /* تحويل الترتيب عمودياً لكي ينزل التوب للأسفل */
            align-items: center;
        }
        .game-box {
            order: 1; /* جعل صندوق اللعبة يظهر أولاً في الأعلى */
        }
        .ghost-leaderboard {
            order: 2; /* جعل الليدربورد تنزل تحت صندوق اللعب */
            width: 100%;
            max-width: 400px;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function soundTick() { playBeep(600, 0.05, "triangle", 0.15); }
    function soundSuccess() { playBeep(880, 0.15, "sine", 0.2); setTimeout(() => playBeep(1320, 0.2, "sine", 0.2), 80); }
    function soundError() { playBeep(220, 0.3, "sawtooth", 0.2); }
    function soundGameOver() { playBeep(440, 0.2, "square", 0.2); setTimeout(() => playBeep(330, 0.4, "square", 0.2), 200); }
    
    function soundNewRecordTrumpet() {
        const notes = [523.25, 659.25, 783.99, 1046.50];
        notes.forEach((freq, index) => {
            setTimeout(() => { playBeep(freq, 0.3, "square", 0.2); }, index * 150);
        });
    }
    ...
    function launchConfettiFestival() {
        ...
        (function frame() {
            confetti({ particleCount: 5, angle: 60, spread: 55, origin: { x: 0, y: 0.8 }, colors: ['#7c3aed', '#a78bfa', '#38bdf8', '#4ade80', '#fbbf24'] });
            confetti({ particleCount: 5, angle: 120, spread: 55, origin: { x: 1, y: 0.8 }, colors: ['#7c3aed', '#a78bfa', '#38bdf8', '#4ade80', '#fbbf24'] });
    ...
    function startRoundGlobalTimer() {
        ...
        roundTimerInterval = setInterval(() => {
            ...
            if (totalRoundTime >= 0) {
                roundTimerCount.textContent = totalRoundTime;
                if (totalRoundTime <= 5 && totalRoundTime > 0) { soundTick(); }
            }
            if (totalRoundTime <= 0) {
                endGameRound();
            }
    ...
    function startTypingCountdown() {
        ...
        wordCountdownInterval = setInterval(() => {
            ...
            if (typingSecondsLeft > 0) {
                countdownUi.textContent = `⏳ اكتب سريعاً! الوقت المتبقي: ${typingSecondsLeft}s`;
                if (typingSecondsLeft <= 4) { soundTick(); }
            } else {
                clearInterval(wordCountdownInterval);
                timeOutFailure();
            }
    ...
    function checkWordSubmission(typed, correct) {
        if (typed.toLowerCase() === correct.toLowerCase() && typed !== "") {
            ...
            if (typingSecondsLeft >= 10) { pointsEarned = 6; speedMessage = "⚡ سرعة شبحية خارقة!"; }
            else if (typingSecondsLeft >= 5) { pointsEarned = 4; speedMessage = "🔋 تركيز رائع وسريع!"; }
